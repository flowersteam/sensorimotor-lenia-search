...
system = Old_lenia(initialization_space=initialization_space, config=lenia_config, device='cuda')
...
performances=Dict()
ctr=0
print(sub_seed*(total_seed//40+1),(sub_seed+1)*(total_seed//40+1))
for key in list_data.keys():
        crea_info=list_data[key]
        ...
        if( (crea_info["is_robust"] and crea_info["is_soliton"] and crea_info["is_long_term_stable"])):
            ...
            if(ctr>=(sub_seed+1)*(total_seed//40+1)):
                break
            
            ctr+=1
            if(ctr-1>=sub_seed*(total_seed//40+1)):
                print(key)
                file=key+".pickle"
                ...
                for i in range(50):
                    ...
                    with torch.no_grad():
                        ...
                        if(crea_is_robust and crea_is_long_term_stable and crea_is_soliton):
                            robu+=1/50
                            speed=avg_speed_test(statistics['activation_center_position'],SX=256,SY=256)
                        else:
                            speed=0
...
with open(os.environ["ALL_CCFRSCRATCH"]+"/sensorimotor_lenia/resources/"+type_expe+"_exploration/tests/"+'perfs_subseed'+str(sub_seed)+'.json', 'w') as f:
