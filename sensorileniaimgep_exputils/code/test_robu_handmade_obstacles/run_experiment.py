...
with open(os.environ["ALL_CCFRSCRATCH"]+"/sensorimotor_lenia/resources/creatures_categoriesV2.json", 'r') as f:
  data = json.load(f)
...
path_hd_obs=os.environ["WORK"]+"/leniaExpe/sensorileniaimgep_exputils/hand_obstacles.pickle"
hd_obs=torch.load(path_hd_obs)    
nb_hd=hd_obs.shape[0]
print("nb of hd obstacles = "+str(nb_hd))
            ...
            if(ctr-1>=sub_seed*(total_seed//10+1)):
                ...
                for i in range(nb_hd):
                    ...
                    with torch.no_grad():
                        ...
                        system.state[0,:,:,1]=torch.flip(hd_obs[i],dims=(1,))
                        ...
                        avg_speed+=speed/nb_hd
                torch.save(list_statistics,os.environ["ALL_CCFRSCRATCH"]+"/sensorimotor_lenia/resources/"+type_expe+"_exploration/tests/"+key+"_stats_tests_hd.pickle")
                perf["speed_obs_hd"]=avg_speed
                perf["robustness_obstacles_hd"]=robu
...
with open(os.environ["ALL_CCFRSCRATCH"]+"/sensorimotor_lenia/resources/"+type_expe+"_exploration/tests/"+'perfs_hd_seed'+str(crea_seed)+'_subseed'+str(sub_seed)+'.json', 'w') as f:
