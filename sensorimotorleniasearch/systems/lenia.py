...
class LeniaStepFFTC(torch.nn.Module):
    ...
    def __init__(self,C, R, T,c0,c1,r,rk, b,w,h, m, s, gn=1,kn=-1, is_soft_clip=False, SX=256, SY=256,speed_x=0,speed_y=0 ,device='cpu'):
        torch.nn.Module.__init__(self)
...
class Lenia_C(torch.nn.Module):
    ...
    def generate_init_state_bis(self):
        init_state = torch.zeros( 1,self.config.SX, self.config.SY,self.config.C, dtype=torch.float64)
        sx, sy = self.initialization.init.shape
        init_hoffset=10
        init_state[:, self.config.SX // 2 - sx // 2:self.config.SX // 2 - sx // 2 + sx,init_hoffset:init_hoffset+sy,0] = torch.flip(self.initialization.init,dims=(1,))
